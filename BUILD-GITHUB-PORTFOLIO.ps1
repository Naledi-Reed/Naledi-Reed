...
try {
    foreach ($project in $manifest.standalone_repositories) {
        ...
        foreach ($file in $files) {
            if ((Get-Content $file.FullName -Raw -ErrorAction SilentlyContinue) -match '(?i)DBD261|database-sql-server|database-systems') {
                throw "Blocked DBD content detected in $($project.source): $($file.FullName)"
            }
        }
    ...
    # Replace profile links with direct links to the new standalone repositories.
